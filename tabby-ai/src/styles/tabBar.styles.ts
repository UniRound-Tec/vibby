/**
 * Everything vibby changes about tabby's own tab bar, as one stylesheet.
 *
 * Restyling from here rather than editing core's SCSS keeps the upstream diff
 * down to the single markup hook in tabHeader.component.pug. It stays a
 * template function rather than becoming a .scss file because the accent
 * colour and both body classes are owned by TypeScript.
 */

export interface TabBarStyleTokens {
    /** Follows the active tabby theme instead of imposing a fixed brand colour */
    accent: string
    /** Body class RailService toggles for the icons-only rail */
    collapsedClass: string
    /** Body class StartupCoverService toggles while the initial route resolves */
    coverClass: string
}

export function tabBarStyles ({ accent, collapsedClass, coverClass }: TabBarStyleTokens): string {
    // every collapsed rule shares this prefix; :is() keeps the block
    // readable where spelling both sides out would double its length
    const collapsed = `body.${collapsedClass} .content:is(.tabs-on-left, .tabs-on-right) > .tab-bar`
    return `
            ...
            body.${collapsedClass} .btn-tab-bar:has(svg.vibby-rail-toggle) {
                background: rgba(128, 128, 128, .2);
                opacity: 1;
            }
            body.${collapsedClass} .content.tabs-on-left,
            body.${collapsedClass} .content.tabs-on-right {
                --side-tab-width: calc(58px * var(--spaciness));
            }
            body.${collapsedClass} .vibby-rail-empty {
                display: none !important;
            }
            ...
            body.${collapsedClass} .window { min-height: 0; }
